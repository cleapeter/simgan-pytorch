...
class ResNetBlock(nn.Module):
    ...
    def __init__(
        self, in_features, num_features=64, kernel_size=3, act_func="LeakyReLU"
    ):
        super().__init__()
...
class Refiner(nn.Module):
    ...
    def __init__(
        ...
        blocks = [
            ResNetBlock(num_features, num_features, kernel_size, act_func)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, act_func="LeakyReLU"):
        ...
        self.conv4 = nn.Conv2d(
            in_channels=32, out_channels=32, kernel_size=1, bias=False, stride=1
        )
        self.conv4_bn = nn.BatchNorm2d(32)
        self.conv5 = nn.Conv2d(
            in_channels=32, out_channels=2, kernel_size=1, stride=1
        )
    ...
    def forward(self, x, labels):
        ...
        # Use labels in embedding layer as condition
        embedding = self.embedding(labels).view(
            labels.shape[0], 1, config.img_height, config.img_width
        )
        # Add embedding as a second channel
        x = torch.cat([x, embedding], dim=1)
        # Conv layer # 1
