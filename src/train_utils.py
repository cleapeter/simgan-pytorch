...
def pretraining_refiner(
    ...
    for i in tqdm(range(num_steps)):
        ...
        if (i % log_steps_images == 0
                or i == num_steps - 1) and plot_path is not None:
            # Create image grid for 10 different digits
            if real_data is not None:
                real_images, real_labels = real_data
                # Compute indices for digits
                syn_idx = [torch.argmin(abs(syn_labels - j))
                           for j in range(10)]
                real_idx = [torch.argmin(abs(real_labels - j))
                            for j in range(10)]
                ...
                selected_images = (selected_syn_images
...
def full_training(
    ...
    for _ in range(num_r_steps):
        ...
        # Get synthetic image batch
        syn_data = utils.get_img_batch(syn_iter, syn_loader)
        syn_iter, syn_images, syn_labels = syn_data
        # Refine synthetic images
        refined_images = refiner(syn_images, syn_labels)
        refined_pred = discriminator(refined_images, syn_labels)
    ...
    for _ in range(num_d_steps):
        ...
        syn_iter, syn_images, syn_labels = syn_data
        # Get real image batch
        real_data = utils.get_img_batch(real_iter, real_loader)
        real_iter, real_images, real_labels = real_data
        ...
        if image_history_buffer is not None:
            history_img_batch = image_history_buffer.get_from_image_history_buffer()
            image_history_buffer.add_to_image_history_buffer(
                refined_images.cpu().data.numpy())

            # Replace half of refined images with images from buffer
            if len(history_img_batch):
                history_img_batch = torch.from_numpy(
                    history_img_batch).to(config.device)
        ...
        # Save losses every log_steps
        if (current_step % log_steps_loss == 0 or
                current_step == config.max_steps - 1):
    ...
    if (current_step % log_steps_images == 0 or
            current_step == config.max_steps - 1) and plot_path is not None:
        ...
        if conditional:
            ...
            syn_idx = [torch.argmin(abs(syn_angle_id - i))
                       for i in [0, 10, 20, 30, 40]]
            real_idx = [torch.argmin(abs(real_angle_id - i))
                        for i in [0, 10, 20, 30, 40]]
        else:
            with torch.no_grad():
    ...
    if (current_step % log_steps_loss == 0 or
            current_step == config.max_steps - 1):
        return (mean_loss_r, mean_loss_r_adv, mean_loss_r_reg,
                mean_loss_d_tot, mean_loss_d_mean, mean_loss_d_real,
                mean_loss_d_refined, mean_acc_d_real, mean_acc_d_refined)
    else:
